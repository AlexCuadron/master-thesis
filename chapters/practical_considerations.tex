...
\cmnt{A potential operational failure is where multiple \tps are behind the same NAT device.
	This would lead to \tps having identical keys from the view of remote \tps. One possible solution
	would be to use a unique \tp identifier instead. Since all such \tps would be under one
	administration domain, assigning unique \tp identifiers upon bootstrapping is feasible. Then,
	the \tps convey their identifiers in the AT header field alongside the destination zone ID. This
...
The control-plane latency overhead would be higher compared to the dense deployment
